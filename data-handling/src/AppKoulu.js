...
function reducer(state, action) {
  switch (action.type) {
    ...
    case "OPPILAAN_NIMI_MUUTTUI": {
      ...
      const tilaKopio = { ...state, tallennetaanko: true };
      tilaKopio.koulut[action.payload.kouluIndex].luokat[
        action.payload.luokkaIndex
      ].oppilaat[action.payload.oppilasIndex].nimi = action.payload.nimi;
      return tilaKopio;
    }
    ...
    case "LISÄÄ_OPPILAS": {
      console.log("Lisää oppilas", action);
      const kopio = { ...state };
      kopio.koulut[action.payload.kouluIndex].luokat[
        action.payload.luokkaIndex
      ].oppilaat.push({ nimi: "oletusnimioppilaalle" });
      return kopio;
    }
    ...
    case "LISÄÄ_KOULU": {
      ...
      return {
        ...state,
        koulut: [...state.koulut, { nimi: "oletusnimi", luokat: [] }],
        tallennetaanko: true,
      };
    }
...
function App() {
  ...
  return (
    <div>
      {appData.tietoAlustettu &&
        appData.koulut.map((koulu, index) => (
          <Koulu kouluIndex={index} koulu={koulu} dispatch={dispatch} />
        ))}
      <button onClick={() => dispatch({ type: "LISÄÄ_KOULU" })}>
        Lisää uus koulu
      </button>
    </div>
  );
